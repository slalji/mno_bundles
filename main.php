...
class Cheka {
	...
	function db_connect(){
		$this->dbcon = mysqli_connect('127.0.0.1','root','roots','selcom_voda_cheka');
...
if ($service == "validate") {
	$cheka->chekaValidate($service);
} elseif ($service == "query") {
	$cheka->queryTransaction($service);
} elseif ($service == "balance") {
	$cheka->getBalance($service);
} elseif ($service == "bundle") {
	$cheka->getBundles($service);
